...
class DistinctiveDataLoader(DataLoader):
    def __init__(self, task_dir, results_dir,do_generate, accuracy_threshold,recall_threshold,min_depth,max_depth):
        ...
        if do_generate:
            self.accuracy_neighbors, \
            self.recall_neighbors = self.generate_distinctive_neighbor(task_dir, results_dir, accuracy_threshold,
                                                                       recall_threshold,min_depth,max_depth)
        else:
            with open(os.path.join(results_dir, "distinctive_neighbors.pkl"), 'rb') as f:
                self.accuracy_neighbors, \
                self.recall_neighbors = pickle.load(f)

    def powerset(self, iterable, min_depth, max_depth):
        s = sorted(iterable)
        return chain.from_iterable(combinations(s, r) for r in range(min_depth, max_depth))

    def generate_distinctive_neighbor(self, dir, save_dir, accuracy_threshold, recall_threshold, min_depth, max_depth):
        ind_dir = dir + '_ind'
        ...
        for r in sorted(relations):
            ...
            for n1 in relation2neighbors[r]:
                ...
                if len(other_neighbors) != 0:
                    potential_set = self.powerset(other_neighbors, min_depth, max_depth)
        ...
        accuracy_neighbors = {self.relation2id[r]: [np.array(
            [self.relation2id[list[i]] for i in range(len(list))])for list in accuracy_neighbors[r]] for r in
                              accuracy_neighbors}
        recall_neighbors = {self.relation2id[r]: [np.array(
            [self.relation2id[list[i]] for i in range(len(list))])for list in recall_neighbors[r]] for r in
                              recall_neighbors}
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        with open(os.path.join(save_dir, "distinctive_neighbors.pkl"), "wb") as f:
            pickle.dump([accuracy_neighbors, recall_neighbors], f)
        return accuracy_neighbors, recall_neighbors
    ...
    def get_subgraph(self, G, neighbor_filter, center_node, query_relation, depth):
        ...
        if query_relation not in neighbor_filter:
            return G_sub
        neighbor_similarity = [[f, len(neighbors.intersection(f)) / len(neighbors.union(f))] for f in
                               neighbor_filter[query_relation]]
    ...
    def get_distinctive_neighbors(self, nodes, query_relations, distinctive_neighbors, head_index_full, tail_index_full,
        ...
        mask=np.zeros(len(relations_next),dtype=np.bool_)

        for idx in np.unique(edges[1]):
            position=np.where(edges[1]==idx)
            position_array=edges[1]==idx
            neigbors=relations_next[position]
            candidate_neighbors=distinctive_neighbors[query_relations[idx]]
            if len(candidate_neighbors):
                similarity=np.array([len(np.intersect1d(candidate_neighbors[i],neigbors))/len(np.union1d(candidate_neighbors[i],neigbors)) for i in range(len(candidate_neighbors))])
                rank=np.argsort(-similarity)
                selected_neighbors=candidate_neighbors[rank[0]]
                m=np.isin(relations_next,selected_neighbors[:,None])
                m=np.logical_and(position_array,m)
                mask=np.logical_or(mask,m)


        mask=torch.from_numpy(mask)
        idd_mask = torch.tensor(relations_next == self.n_rel * 2)
        mask = mask if complement == False else torch.logical_not(mask)
        mask = torch.logical_or(mask, idd_mask)
        filtered_indexes = torch.where(mask == True)[0]
